...
std::bitset<3000003> isPrime; std::vector<int> primes;
class Solution {
    void buildPrimeArray(void) {
        isPrime[0] = isPrime[1] = 1;
        for (int i = 2; (i * i) <= 3000003; ++i) if (!isPrime[i]) for (int j = (i * i); j <= 3000003; j += i) isPrime[j] = true;
        for (int i = 2; i <= 3000003; ++i) if (!isPrime[i]) primes.push_back(i);
        debug(primes.size(), primes[200007]);
    }
