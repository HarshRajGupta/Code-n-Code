...
#define __MAIN__                                                              \
	...
	{                                                                         \
		...
		if (preCompute ())                                                    \
			std::cerr << "  🤖 pC: "                                        \
					  << ((long double)std::chrono::duration_cast<            \
							 ...
							 / 1e9                                            \
					  << " sec 🤖\n";                                         \
		int32_t __TEST_CASES__;                                               \
		std::cin >> __TEST_CASES__;                                           \
		std::cerr << "   🎯 Test Cases: #" << __TEST_CASES__ << " 🎯\n"     \
				  << std::endl;                                               \
		for (int32_t __TEST_CASE__ = 1; __TEST_CASE__ <= __TEST_CASES__;      \
			 ++__TEST_CASE__)                                                 \
			{                                                                 \
				std::cerr << "\t<== "                                         \
...
template <typename T>
void
__PRINT_VARIABLE (const std::queue<T> &x)
{ // queue
	auto f = x;
	std::cerr << '[';
	while (!f.empty ())
...
template <typename T>
void
__PRINT_VARIABLE (const std::priority_queue<T> &x)
{ // max heap
	auto f = x;
	std::cerr << '[';
	while (!f.empty ())
...
void
__PRINT_VARIABLE (
	const std::priority_queue<T, std::vector<T>, std::greater<T> > &x)
{ // min heap
	auto f = x;
	std::cerr << '[';
	while (!f.empty ())
...
template <typename T>
void
__PRINT_VARIABLE (const std::vector<T> &x)
{
	std::cerr << '[';
...
void
__PRINT_VARIABLE (const std::array<T, SIZE> &x)
{
	std::cerr << '[';
	unsigned long long f = 0;
	for (const auto &i : x)
...
template <typename T>
void
__PRINT_VARIABLE (const std::forward_list<T> &x)
{
	std::cerr << '[';
	unsigned long long f = 0;
	for (const auto &i : x)
...
template <typename T>
void
__PRINT_VARIABLE (const std::list<T> &x)
{
	std::cerr << '[';
	unsigned long long f = 0;
	for (const auto &i : x)
...
template <typename T>
void
__PRINT_VARIABLE (const std::set<T> &x)
{
	std::cerr << '[';
	unsigned long long f = 0;
	for (const auto &i : x)
...
template <typename T>
void
__PRINT_VARIABLE (const std::multiset<T> &x)
{
	std::cerr << '[';
	unsigned long long f = 0;
	for (const auto &i : x)
...
template <typename T>
void
__PRINT_VARIABLE (const std::unordered_set<T> &x)
{
	std::cerr << '[';
	unsigned long long f = 0;
	for (const auto &i : x)
...
template <typename T>
void
__PRINT_VARIABLE (const std::unordered_multiset<T> &x)
{
	std::cerr << '[';
	unsigned long long f = 0;
	for (const auto &i : x)
...
template <typename T, typename... V>
void
__DEBUG_VAR (std::queue<std::string> &s, const T t, V... v)
{
	std::cerr << '\t' << s.front () << ": ";
	s.pop ();
	__PRINT_VARIABLE (t);
